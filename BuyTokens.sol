...
import {IssueRWAT} from "IssueRWAT.sol";
...
contract SecurePropertyToken is ERC721URIStorage, Ownable {
    ...
    uint256 public tokenPriceUSDC; // price in USDC (smallest unit 6 decimals)
    uint256 public tokenPriceUSD;
    uint256 public totalTokens;
    bool public propertyTokenized;
    ...
    IssueRWAT public issueRWAT;

    constructor(address issueRWATaddress) ERC721("RealWorldAssetToken", "RWAT") Ownable(msg.sender) {
        issueRWAT = IssueRWAT(issueRWATaddress);
        property = Property ({
        ...
        propertyTokenized = issueRWAT.propertyTokenized();
        issuedTokens = issueRWAT.issuedTokens();
        totalTokens = issueRWAT.totalTokens();
    ...
    function availableTokens() public view returns (uint256) {
        ...
        for (uint256 i = 1; i <= totalTokens; i++) {
            if (issueRWAT.ownerOf(i) == address(0) || soldTokens[i]) {
                continue;
            }
            available++;
        }
